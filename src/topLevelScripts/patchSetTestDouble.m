...
outDir   = ensureDir([home 'evaluation/oldDoubleHNM/']);
...
if ~exist([outDir 'chosenCategories.mat'],'file')
    organicC3File = [patchDir 'organicC3OldDoubleHNM.isolated.mat'];
    inorganicC3File = [patchDir 'inorganicC3OldDoubleHNM.isolated.mat'];
...
for i = 1:length(type1)
    for j = 1:length(type2)
        c3Files{i,j} = regexprep(c2Files,'kmeans.c2', ...
	                         [type1{i} 'OldDoubleHNM.' type2{j} '.c3']);
        patchFile{i,j} = [patchDir type1{i} 'C3OldDoubleHNM.' type2{j} '.mat'];
